...
- **业务背景**：C端消费者在浏览某店铺商品列表或商品详情时，如果该店铺已暂停营业（`status != 1`） or 不存在，继续展示商品甚至允许下单将引发严重业务纠纷。
- **校验逻辑**：在 `GoodsCustomerServiceImpl` 的商品列表查询（`getGoodsByShopId`）和单品详情查询（`getGoodsById`）中，引入对所属店铺的联动检索：
  1. **走缓存查询**：联动查询统一改走 `CacheClient` 逻辑过期缓存读取（缓存键前缀 `"o2o:shop:detail:"`），保障高并发下零 SQL 穿透。
  2. **净化回调与外层校验**：数据库 Fallback 回调仅做 `shopMapper::selectById` 查询（返回 null），由业务层在缓存方法返回后统一在外部拦截校验：
     - 若店铺不存在，抛出 `404` 异常（提示“目标店铺不存在”）。
     - 若店铺的 `status` 不为 `1`（营业中），抛出 `400` 异常（提示“该店铺已暂停营业”）。
  3. **缓存与状态解耦设计**：对于单品详情，商品详情缓存与店铺状态独立，商品详情查询完毕后实时从 Redis 读取店铺缓存，实现了店铺打烊/恢复营业时的秒级联动同步，避免了商品缓存的频繁物理失效。
...
| :--- | :--- | :--- | :--- |
| **店铺详情** | `getShopById` | **逻辑过期 + 异步线程池重建** | 店铺信息为核心超热点，不允许任何读取卡顿。缓存不设置 Redis 物理 TTL，由 JSON 包装类维护逻辑过期时间（当前优化配置为 12 小时）。过期时，利用 Redisson 抢锁，仅放行一个异步线程查库更新，其他线程直接返回旧缓存，保障读请求 100% 毫秒级命中。 |
...
在 `o2o-shop-service` 的缓存底层设计中，各维度缓存的时间参数经过了精确的调优与分工，具体配置对照如下：

| 缓存维度 | 核心策略 | 关键时间参数项 | 当前配置值及单位 | 设计考量与业务作用 |
| :--- | :--- | :--- | :--- | :--- |
| **店铺详情 (C端)** | 逻辑过期 | 逻辑过期时间 (Logical TTL) | **`12 小时`** (12 Hours) | 店铺数据变动极少，且配合 B端修改主动失效，设为 12 小时自愈期可大幅降低系统后台查库压力。 |
| | | 冷启动等锁时间 (Wait Time) | **`5 秒`** (5 Seconds) | 缓存彻底为空时的并发请求排队等锁时限，超时自动降级读库，保障高可用。 |
| | | 穿透空值缓存时间 (Null TTL) | **`2 分钟`** (2 Minutes) | 当查询不存在的店铺时，Redis 写入 `"null"` 占位符的物理生命期，阻断恶意穿透。 |
| **商品列表 (C端)** | 旁路缓存 | 基础物理生存期 (Physical TTL) | **`30 分钟`** (30 Minutes) | 自动淘汰冷门商家的商品数据，释放 Redis 内存。 |
| | | 随机生存抖动范围 (Jitter Range) | **`0 ~ 300 秒`** (0~5 Minutes) | 在 30 分钟基础上随机累加，平滑过期时间点，防止缓存雪崩。 |
| | | 重建抢锁等待时间 (Mutex Wait) | **`3 秒`** (3 Seconds) | 尝试获取重建锁的最大时限，抢不到则休眠 50ms 后自旋重试。 |
| | | 穿透空值缓存时间 (Null TTL) | **`2 分钟`** (2 Minutes) | 店铺无商品或查询不存在的店铺时，缓存空值防穿透。 |
| **单品详情 (C端)** | 旁路缓存 | 基础物理生存期 (Physical TTL) | **`30 分钟`** (30 Minutes) | 物理淘汰冷门单品，防止百万级商品撑爆 Redis 内存。 |
| | | 随机生存抖动范围 (Jitter Range) | **`0 ~ 300 秒`** (0~5 Minutes) | 平滑单品过期时间点，防雪崩。 |
| | | 重建抢锁等待时间 (Mutex Wait) | **`3 秒`** (3 Seconds) | 看门狗自动续期下，尝试抢占重建锁的最大等待时限。 |
| | | 穿透空值缓存时间 (Null TTL) | **`2 分钟`** (2 Minutes) | 商品不存在时缓存空值防穿透。 |
| **线程池 (CacheClient)** | 异步重建 | 闲置生存时间 (KeepAlive) | **`60 秒`** (60 Seconds) | 异步刷新线程池中非核心空闲线程的生命期，超时自动销毁。 |
...
### 9.3 进阶 LBS 空间检索：GEO 结构与检索流程
针对 O2O 业务特有的“同城 3 公里范围内店铺检索与距离排序”功能，项目底层依托 **GEO 空间地理索引结构** 实现：
*   **内部机制**：Redis 内部将经纬度坐标（Longitude, Latitude）转换为 Geohash 编码，并以 ZSet 结构进行物理落盘存储，使得空间检索的复杂度控制在 $O(\log N + M)$，其中 $N$ 为集合中店铺总数，$M$ 为检索范围内的店铺数量。
*   **物理检索与详情解耦**：Redis GEO 仅负责空间拓扑过滤，返回符合条件的 Shop ID 列表与距离，具体店铺的展示属性则交由店铺详情缓存（`o2o:shop:detail:{id}`）加载，实现“索引”与“实体数据”的物理解耦。

---

### 9.4 核心性能设计：分类隔离 Key 方案

由于 Redis Geo 命令（如 `GEOSEARCH`）仅能根据地理空间坐标 and 半径进行单一维度的物理距离过滤，不支持复杂的属性筛选（如商品分类）。为保障高性能的联合检索，本项目采用 **分类隔离 Key 方案**。

#### 1) 为什么不使用“单 GEO Key + 数据库过滤”？
*   **无效查询与带宽浪费**：如果把所有店铺放在一个 Key 中，当用户搜索“附近的美食店”时，Redis 可能会查出附近 3 公里内的所有店铺（如 800 家），传回 Java 后再通过 MySQL 过滤分类，这会产生严重的“无效 ID 查询与丢弃”问题，浪费大量网络带宽和 CPU。
*   **分页截断与数据丢失 Bug**：若限制 Redis 仅返回最近的 20 个店铺，但这 20 个店铺里可能一个“美食”店都没有，而美食店在第 21 个位置。过滤后就会返回空列表，导致“附近无美食”的假象，产生严重的功能 Bug。

#### 2) 分类 Key 体系设计
*   **全局地理位置索引 Key**：`o2o:shop:geo:all`（保存所有营业中店铺的坐标，用于首页无分类推荐）。
*   **分类隔离地理位置索引 Key**：`o2o:shop:geo:category:{category}`（如 `o2o:shop:geo:category:美食`，仅保存该分类下营业中店铺的坐标，专用于按分类过滤检索）。
*   **空间复杂度与内存**：Redis Geo 底层为 ZSet，仅保存店铺 ID 字符和 52 位 Geohash 分数。假设平台有 10,000 家营业店铺，即使在全局 Key 和分类 Key 中双重存储，总内存开销也仅为 **2.6 MB 左右**，内存开销极低，却换取了读性能的极致提升。

---

### 9.5 核心一致性设计：商户更新双写与事务同步

为了确保 Redis 中存放的 LBS 数据与 MySQL 数据库保持强一致，在商家新建、修改、关店或删除店铺时，必须对 Redis GEO 进行精细的双写控制。

#### 1) 状态变更规则矩阵
*   **新建店铺**：若 `status = 1`（营业中），同步写入 `all` Key 和对应的 `category` Key；若 `status = 0`（休息中），不写入，省内存。
*   **下架/关店**：若 `status` 由 `1` 变为 `0`，从全局 `all` Key 和对应的 `category` Key 中安全移除（`GEOREMOVE`）。
*   **重新营业**：若 `status` 由 `0` 变为 `1`，重新写入 `all` Key 和新 `category` Key。
*   **修改分类/坐标**：更新全局 Key 坐标；若分类改变，从旧分类 Key 移除并添加至新分类 Key。
*   **删除店铺**：从全局及对应分类 Key 中移除。

#### 2) 代码级事务提交后写缓存（Transaction-Bound Action）
在 Spring Boot 的 Service 中，必须确保 **MySQL 事务提交成功后，才操作 Redis**，防止事务回滚导致 Redis 产生脏数据或并发脏读。
*   **实现机制**：使用 `TransactionSynchronizationManager.registerSynchronization` 注册事务同步器，在 `afterCommit()` 回调中同步执行 Redis Geo 的增删改及详情缓存的失效操作。

---

### 9.6 技术问答深度解析：GEO 缓存与店铺详情缓存的协同机制

#### ❓ 问题：为什么已经缓存了店铺详情（Shop Detail），还需要单独缓存 GEO 空间位置？直接从 Shop 详情缓存里取值不行吗？

#### 💡 深度解答：
1.  **查询能力本质不同（Key-Value 查找 vs 空间索引检索）**：
    *   **店铺详情缓存**（如 `o2o:shop:detail:101`）属于 **Key-Value 键值对映射**。它只能解决“已知店铺 ID，获取其具体信息”的单点查询，**无法进行空间范围过滤和距离排序**。
    *   如果只用详情缓存，当用户检索附近店铺时，系统必须遍历 Redis 中所有的店铺详情 Key（成千上万个），反序列化并计算距离。这无异于 Redis 层的“全表扫描”，会导致 Redis 线程阻塞和 CPU 暴满。
    *   **GEO 缓存**（如 `o2o:shop:geo:all`）是专门的 **空间几何索引**，它将经纬度转化为二进制 Geohash 并排好序。它能解决“给定坐标，快速找出半径 3 公里内的店铺 ID 列表”的问题，时间复杂度仅为 $O(\log N + M)$，速度极快。
2.  **经典的“索引 ➡️ 查表”协同机制**：
    *   在一次完整的附近店铺检索中，两类缓存像数据库的“索引”与“主表数据”一样协同工作：
        *   **第一步：空间检索（查索引）**：利用 Redis GEO 缓存，在微秒级内根据经纬度筛选出附近店铺的 ID 列表和算好的距离（如 `[101, 105]`）。
        *   **第二步：详情装配（查主表）**：拿着 ID 列表，点对点地从店铺详情缓存（`o2o:shop:detail:*`）中读取店名、Logo 等静态展现字段。
    *   这种设计既保证了空间检索的高性能，又避免了在 GEO 索引中存入店名、Logo等大字段导致 Redis 内存浪费，实现了架构层面的职责分离。
